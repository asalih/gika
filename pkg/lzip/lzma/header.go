...
func (h *header) unmarshalBinary(data []byte) error {
	...
	// properties
	var err error
	if h.properties, err = PropertiesForCode(93); err != nil {
		return err
	}

	basePower := int(data[5] & 0x1F)
	subtractionNumerator := int((data[5] & 0xE0) >> 5)

	// dictionary capacity
	h.dictCap = (1 << basePower) - subtractionNumerator*(1<<(basePower-4))
	h.size = -1
